...
class Trainer(object):
    def __init__(self, args, data_config):
        ...
        genre = args.genre
        self.train_set, self.val_set, self.test_set = self.build_dataset(
            data_config, genre)
    ...
    def calc_loss(self, span_score, ner_ids):
        ...
        loss = F.cross_entropy(span_score.reshape(-1, num_cls),
                               ner_ids.reshape(-1), ignore_index=0, reduction='sum')
        return loss / num_ner
    ...
    def ner_pred(self, pred_score, sent_lens, ner_vocab=None):
        ...
        # (b, t, t)
        span_max_score = pred_score.detach().gather(
            dim=-1, index=type_idxs.unsqueeze(-1)).squeeze(-1)
        final = []
