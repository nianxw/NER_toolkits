...
from tool_utils.vocabulary import Vocabulary
...
class CNerTokenizer:
    ...
    def get_label(self):
        label_path = os.path.join(self.data_dir, 'label.json')
        if os.path.exists(label_path):
            return json.load(open(label_path, 'r', encoding='utf8'))
        else:
            label_set = set()
            with open(os.path.join(self.data_dir, 'train.json'), 'r') as fr:
                for line in fr:
                    line = json.loads(line.strip())
                    labels = line['labels']
                    for l in labels:
            ...
            json.dump(open(label_path, 'w', encoding='utf8'), label2id, ensure_ascii=False)
            return label2id

    def tokenize(self, text):
        _tokens = []
        for c in text:
            if self.vocab.has_word(c):
                _tokens.append(c)
            else:
                _tokens.append('[UNK]')
        return _tokens
    ...
    def convert_tokens_to_ids(self, tokens):
        return [self.vocab.to_index(token) for token in tokens]
