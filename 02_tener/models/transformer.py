...
class TransformerLayer(nn.Module):
    def __init__(self, self_attn, config):
        ...
        self.after_norm = config.after_norm

        self.ffn = nn.Sequential(nn.Linear(config.d_model, 2*config.d_model),
                                 nn.ReLU(),
...
class TransformerEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self_attn = RelativeMultiHeadAttn(config)
        self.layers = nn.ModuleList(
            [TransformerLayer(deepcopy(self.self_attn), config) for _ in range(config.num_layers)])
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            x = layer(x, mask)  # [bn, seq_len, d_model], [bn, seq_len]
        return x
