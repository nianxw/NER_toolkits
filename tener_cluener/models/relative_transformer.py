...
class RelativeMultiHeadAttn(nn.Module):
    def __init__(self, config):
        ...
        """
        super().__init__()
        d_model, n_head, dropout = config.d_model, config.n_head, config.dropout
    ...
    def forward(self, x, mask):
        ...
        q = q.view(batch_size, max_len, self.n_head, -1).transpose(1, 2)  # b,n,l,d
        ...
        rw_head_q = q + self.r_r_bias[:, None]  # b x n x l x d + (n, 1, d)
        AC = torch.einsum('bnqd,bnkd->bnqk', [rw_head_q, k])  # b x n x l x d, n是head   (Q+v)*K
